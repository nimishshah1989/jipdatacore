# Data Engine — Architecture Document v2.0
**Status:** In Build (Forge OS)
**Total issues resolved:** v1.0→v1.9.1: 35 critical, 53 moderate, 23 minor. Pre-build EC2 inspection (2026-04-05) triggered v2.0 shift.
**Review history:** v1.0→v1.9.1 (infrastructure hardening) → v2.0 (Real-world EC2 Data Engine Build).

### v2.0 Change Log (EC2 Reality Shift)
| # | Severity | What changed | Source |
|---|----------|--------------|--------|
| 1 | CRITICAL | **Mutual Fund data source inversion**: `mf_pulse` Docker DB found to contain 13,380 funds (not 535) with 25.8M NAV rows and 2M+ holdings. `mf_engine` deprecated as primary source. | EC2 Inspection |
| 2 | CRITICAL | **Morningstar API single-endpoint strategy**: Instead of 2 separate APIs, we use the existing fund detail API `IdType/Identifier` mapped via the 5,489 equity funds already in the database. Filter: `Accumulated` + `Reg` in name. | PRD revision |
| 3 | MODERATE | **Equity OHLCV Migration**: `compass_stock_prices` dates (VARCHAR) and values (DOUBLE) explicitly typed for migration via Alembic scripts. | EC2 Inspection |
| 4 | MINOR | **Global ETF data**: Confirmed local Stooq data requires classification mapping mapping before ingestion. | Local Inspection |
...
- **Install PgBouncer**: Deploy PgBouncer locally on EC2 (or separate container) to provide connection pooling (`pool_mode = transaction`, max 1000 client_connections mapping to 50 server_connections) since the Data Engine will act as the single backend for all applications.
...
- **Migration Source**: `compass_stock_prices` in RDS `fie_v3` (1.4M rows) — fields are DOUBLE and DATE is VARCHAR, requires pipeline correction before insert.

### 4.2 Mutual Funds
- **Core Strategy**: Equity category, `Accumulated` (Growth) distribution status, `Regular` plan only (~450-550 target funds)
- AMFI daily NAV: https://www.amfiindia.com/spages/NAVAll.txt (Primary Source for NAV)
- AMFI monthly AUM data
- **Morningstar API**: 1 API ONLY (fund detail endpoint). Call `{IdType}/{Identifier}?datapoints=Name,CategoryName,BroadCategoryGroup,NetExpenseRatio,ManagerName,TotalNetAssets...` filtered by our target equity universe.
- **Migration Source**: Docker Postgres `fie2-db-1`. The `mf_pulse` database contains:
  - `fund_master`: 13,380 funds with detailed AMFI/Morningstar mapping.
  - `nav_daily`: 25.8M rows.
  - `fund_holding_detail`: 2M+ rows.
  - (The legacy `mf_engine` is deprecated as primary data source).
...
- **Migration Source**: Local files in `/home/ubuntu/global-pulse/` (.txt Stooq format). Requires mapping Stooq symbols to instrument types.
...
docker exec fie2-db-1 pg_dump -U fie mf_pulse -t nav_daily \
  --no-owner --no-privileges -f /home/ubuntu/nav_daily_backup.sql

# Equity prices RDS — read from dump
PGPASSWORD=Nimish1234 pg_dump -h fie-db.c7osw6q6kwmw.ap-south-1.rds.amazonaws.com -U fie_admin -d fie_v3 -t compass_stock_prices \
  --no-owner --no-privileges -f /home/ubuntu/compass_stock_prices_backup.sql
```
...
|--------|--------|------|--------|
| fie2-db-1 `mf_pulse.nav_daily` | de_mf_nav_daily | ~5M (filtered) | Filter for target equity universe, map mstar_id. Drop rows with nav <= 0 |
| EC2 RDS `fie_v3.compass_stock_prices` | de_equity_ohlcv | 1.4M | Fix VARCHAR→DATE, DOUBLE→NUMERIC, map ticker → instrument_id |
| fie2-db-1 `mf_pulse.fund_master` | de_mf_master | 13,380 | Map 44 Morningstar columns (Name, ISIN, benchmarks, expense_ratio) |
| fie2-db-1 `mf_pulse.fund_holding_detail` | de_mf_holdings | 2.0M | Directly map to new holdings table. Include sector/weight_pct |
| EC2 RDS `client_portal.cpp_*` | de_client_* | TBD | Encrypt PAN/phone/email before insert, compute HMAC blind indexes |
| EC2 RDS `fie_v3.index_constituents` | de_index_constituents | 4,638 | Map ticker to instrument_id |
