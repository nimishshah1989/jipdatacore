...
).replace("instrument_id", "mstar_id").replace("'equity'", "'mf'")
...
async def compute_equity_rs(benchmarks: list[str] = None, lookback_start: str = "2015-01-01", compute_start: str = "2016-04-01"):
    ...
    for bm in benchmarks:
        ...
        async with engine.begin() as conn:
            ...
            await conn.execute(sa.text(EQUITY_RS_SQL), {"bm": bm, "lookback_start": lookback_start, "compute_start": compute_start})
        print(f"    Done in {time.time()-t0:.0f}s", flush=True)
    await engine.dispose()


async def compute_mf_rs(benchmarks: list[str] = None):
    benchmarks = benchmarks or BENCHMARKS
    import psycopg2
    ...
    for bm in benchmarks:
        ...
        cur.execute(MF_RS_SQL.replace(":bm", "%s").replace(":lookback_start", "%s").replace(":compute_start", "%s"),
                    (bm, "2015-01-01", "2016-04-01", bm))
